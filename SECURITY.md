...
|---|---|---|---|
| VSP-SEC-002 | 2026-09-08 | Relayer native-value drain: the relay backend forwarded the signed request's `value` from the relayer's own wallet (`build_transaction({"value": req.value})`) and nothing constrained the target, so an attacker-signed request with a non-zero value and an attacker-chosen `to` drained the relayer EOA for the cost of one relay fee. Reported privately by [@ibnu76](https://github.com/ibnu76) under this policy. | Fixed 2026-09-10 — backend rejects `value != 0` and non-protocol targets before spending gas, pins `value = 0` on submit, adds a per-IP relay limit and an hourly spend-rate circuit breaker; forwarder v5 reverts on-chain on non-zero value and non-allow-listed targets (app PR #12; regressions `test_F1_*`) |
